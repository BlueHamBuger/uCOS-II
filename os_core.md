...
void  OS_Sched (void) //任务调度函数
{
	...
	if (OSIntNesting == 0u)                            /* Schedule only if all ISRs done and ...       */
	{
		if (OSLockNesting == 0u)                       /* ... scheduler is not locked                  */
		{
			...
			if (OSPrioHighRdy != OSPrioCur)            /* No Ctx Sw if current task is highest rdy 不是正在进行的任务    */
			{
#if OS_TASK_PROFILE_EN > 0u
				OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task     切换次数加+ */
#endif
...
// tcb初始化
//初始化任务控制块TCB(优先级、栈顶指针、栈底指针、任务标志符、堆栈容量、扩展指针、选择项
